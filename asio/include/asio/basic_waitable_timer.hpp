...
template <typename Clock,
    typename WaitTraits = ASIO_LIBNS::wait_traits<Clock>,
 ...
 *
 * Most applications will use one of the ASIO_LIBNS::steady_timer,
 * ASIO_LIBNS::system_timer or ASIO_LIBNS::high_resolution_timer typedefs.
 *
 ...
 * ASIO_LIBNS::steady_timer timer(my_context);
 *
 ...
 * @code
 * void handler(const ASIO_LIBNS::error_code& error)
 * {
 *   if (!error)
 *   {
 *     // Timer expired.
 ...
 * }
 *
 * void on_timeout(const ASIO_LIBNS::error_code& e)
 * {
 *   if (e != ASIO_LIBNS::error::operation_aborted)
 *   {
 ...
 * @endcode
 *
 * @li The ASIO_LIBNS::basic_waitable_timer::expires_after() function
 * cancels any pending asynchronous waits, and returns the number of
 * asynchronous waits that were cancelled. If it returns 0 then you were too
 * late and the wait handler has already been executed, or will soon be
 * executed. If it returns 1 then the wait handler was successfully cancelled.
 *
 * @li If a wait handler is cancelled, the ASIO_LIBNS::error_code passed to
 * it contains the value ASIO_LIBNS::error::operation_aborted.
 */
...
public:
    ...
    : impl_(0, ex)
  {
    ...
    ASIO_LIBNS::detail::throw_error(ec, "expires_at");
  }
    ...
    : impl_(0, ex)
  {
    ...
    ASIO_LIBNS::detail::throw_error(ec, "expires_after");
  }
   ...
   * be invoked with the ASIO_LIBNS::error::operation_aborted error code.
   *
   ...
   * error code that indicates the successful completion of the wait operation.
   */
  std::size_t cancel(ASIO_LIBNS::error_code& ec)
  {
   ...
   * ASIO_LIBNS::error::operation_aborted error code.
   *
   * Cancelling the timer does not change the expiry time.
   *
  ...
  std::size_t cancel_one()
  {
    ...
    ASIO_LIBNS::detail::throw_error(ec, "cancel_one");
    return s;
  }
   ...
   * error code that indicates the successful completion of the wait operation.
   */
  std::size_t cancel_one(ASIO_LIBNS::error_code& ec)
  {
  ...
  std::size_t expires_from_now(const duration& expiry_time)
  {
    ...
    std::size_t s = impl_.get_service().expires_from_now(
        impl_.get_implementation(), expiry_time, ec);
    ASIO_LIBNS::detail::throw_error(ec, "expires_from_now");
    return s;
  }
   ...
   * code ASIO_LIBNS::error::operation_aborted.
   *
  ...
  ASIO_INITFN_AUTO_RESULT_TYPE_PREFIX(
      WaitToken, void (ASIO_LIBNS::error_code))
  async_wait(
