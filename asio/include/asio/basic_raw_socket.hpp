...
public:
   ...
   * @code socket.send(ASIO_LIBNS::buffer(data, size)); @endcode
   ...
   * To send a single data buffer use the @ref buffer function as follows:
   * @code
   * ASIO_LIBNS::ip::udp::endpoint destination(
   *     ASIO_LIBNS::ip::address::from_string("1.2.3.4"), 12345);
   * socket.send_to(ASIO_LIBNS::buffer(data, size), destination);
   * @endcode
   * See the @ref buffer documentation for information on sending multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
      ...
      const endpoint_type& destination)
  {
    ...
    ASIO_LIBNS::detail::throw_error(ec, "send_to");
    return s;
  }
   ...
   *     ASIO_LIBNS::buffer(data, size), destination, handler);
   * @endcode
   * See the @ref buffer documentation for information on sending multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
   ...
   * follows:
   * @code socket.receive(ASIO_LIBNS::buffer(data, size)); @endcode
   ...
   * follows:
   * @code
   * ASIO_LIBNS::ip::udp::endpoint sender_endpoint;
   * socket.receive_from(
   *     ASIO_LIBNS::buffer(data, size), sender_endpoint);
   * @endcode
   * See the @ref buffer documentation for information on receiving into
   * multiple buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
      ...
      endpoint_type& sender_endpoint)
  {
    ...
    ASIO_LIBNS::detail::throw_error(ec, "receive_from");
    return s;
  }
   ...
   *     ASIO_LIBNS::buffer(data, size), 0, sender_endpoint, handler); @endcode
   * See the @ref buffer documentation for information on receiving into
   * multiple buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
   *
