...
/**
 * The ASIO_LIBNS::ip::basic_endpoint class template describes an endpoint that
...
public:
...
#else
  typedef ASIO_LIBNS::detail::socket_addr_type data_type;
#endif
   ...
   * @code
   * ASIO_LIBNS::ip::tcp::endpoint ep(ASIO_LIBNS::ip::tcp::v4(), 1234);
   * @endcode
   *
   * To specify an IPv6 UDP endpoint for port 9876, use:
   * @code
   * ASIO_LIBNS::ip::udp::endpoint ep(ASIO_LIBNS::ip::udp::v6(), 9876);
   * @endcode
   */
  ...
  basic_endpoint(const ASIO_LIBNS::ip::address& addr,
      port_type port_num) ASIO_NOEXCEPT
    : impl_(addr, port_num)
  {
  }
  ...
  ASIO_LIBNS::ip::address address() const ASIO_NOEXCEPT
  {
    return impl_.address();
  }

  /// Set the IP address associated with the endpoint.
  void address(const ASIO_LIBNS::ip::address& addr) ASIO_NOEXCEPT
  {
    impl_.address(addr);
  }
...
private:
  ...
  ASIO_LIBNS::ip::detail::endpoint impl_;
};
 ...
 *
 * @relates ASIO_LIBNS::ip::basic_endpoint
 */
...
template <typename InternetProtocol>
struct hash<ASIO_LIBNS::ip::basic_endpoint<InternetProtocol> >
{
  std::size_t operator()(
      const ASIO_LIBNS::ip::basic_endpoint<InternetProtocol>& ep)
    const ASIO_NOEXCEPT
  {
    std::size_t hash1 = std::hash<ASIO_LIBNS::ip::address>()(ep.address());
