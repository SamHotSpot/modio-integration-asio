...
std::string address_v6::to_string() const
{
  ...
  char addr_str[ASIO_LIBNS::detail::max_addr_v6_str_len];
  const char* addr =
    ASIO_LIBNS::detail::socket_ops::inet_ntop(
        ASIO_OS_DEF(AF_INET6), &addr_, addr_str,
        ASIO_LIBNS::detail::max_addr_v6_str_len, scope_id_, ec);
  if (addr == 0)
...
std::string address_v6::to_string(ASIO_LIBNS::error_code& ec) const
{
  char addr_str[ASIO_LIBNS::detail::max_addr_v6_str_len];
  const char* addr =
    ASIO_LIBNS::detail::socket_ops::inet_ntop(
...
bool operator==(const address_v6& a1, const address_v6& a2) ASIO_NOEXCEPT
{
  ...
  return memcmp(&a1.addr_, &a2.addr_,
      sizeof(ASIO_LIBNS::detail::in6_addr_type)) == 0
...
bool operator<(const address_v6& a1, const address_v6& a2) ASIO_NOEXCEPT
{
  ...
  int memcmp_result = memcmp(&a1.addr_, &a2.addr_,
      sizeof(ASIO_LIBNS::detail::in6_addr_type));
  if (memcmp_result < 0)
    return true;
  if (memcmp_result > 0)
    return false;
