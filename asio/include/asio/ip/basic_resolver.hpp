...
public:
   ...
   * will be invoked with the ASIO_LIBNS::error::operation_aborted error code.
   */
  void cancel()
  {
  ...
  results_type resolve(const query& q)
  {
    ...
    ASIO_LIBNS::detail::throw_error(ec, "resolve");
    return r;
  }
   ...
   */
  results_type resolve(const query& q, ASIO_LIBNS::error_code& ec)
  {
   ...
   */
  results_type resolve(ASIO_STRING_VIEW_PARAM host,
      ASIO_STRING_VIEW_PARAM service, ASIO_LIBNS::error_code& ec)
  {
  ...
  results_type resolve(const protocol_type& protocol,
      ASIO_STRING_VIEW_PARAM host, ASIO_STRING_VIEW_PARAM service,
      resolver_base::flags resolve_flags, ASIO_LIBNS::error_code& ec)
  {
    basic_resolver_query<protocol_type> q(
        protocol, static_cast<std::string>(host),
        static_cast<std::string>(service), resolve_flags);
   ...
   * @code void(ASIO_LIBNS::error_code, results_type) @endcode
   */
  template <
  ...
  ASIO_INITFN_AUTO_RESULT_TYPE_PREFIX(ResolveToken,
      void (ASIO_LIBNS::error_code, results_type))
  async_resolve(const query& q,
      ASIO_MOVE_ARG(ResolveToken) token
        ASIO_DEFAULT_COMPLETION_TOKEN(executor_type))
    ASIO_INITFN_AUTO_RESULT_TYPE_SUFFIX((
      ASIO_LIBNS::async_initiate<ResolveToken,
        void (ASIO_LIBNS::error_code, results_type)>(
          declval<initiate_async_resolve>(), token, q)))
  {
    return ASIO_LIBNS::async_initiate<ResolveToken,
      void (ASIO_LIBNS::error_code, results_type)>(
   ...
   * function is guaranteed to return a non-empty range.
   */
  results_type resolve(const endpoint_type& e, ASIO_LIBNS::error_code& ec)
  {
...
private:
  ...
  public:
        ...
        const Query& q) const
    {
      ...
      ASIO_LIBNS::detail::non_const_lvalue<ResolveHandler> handler2(handler);
...
# if defined(ASIO_WINDOWS_RUNTIME)
  ASIO_LIBNS::detail::io_object_impl<
    ASIO_LIBNS::detail::winrt_resolver_service<InternetProtocol>,
    Executor> impl_;
# else
  ASIO_LIBNS::detail::io_object_impl<
    ASIO_LIBNS::detail::resolver_service<InternetProtocol>,
    Executor> impl_;
# endif
};
