  ...
  : data_()
{
  ...
  if (family == ASIO_OS_DEF(AF_INET))
  {
    data_.v4.sin_family = ASIO_OS_DEF(AF_INET);
    data_.v4.sin_port =
      ASIO_LIBNS::detail::socket_ops::host_to_network_short(port_num);
    data_.v4.sin_addr.s_addr = ASIO_OS_DEF(INADDR_ANY);
  }
  else
  {
...
endpoint::endpoint(const ASIO_LIBNS::ip::address& addr,
  ...
  : data_()
{
  ...
  if (addr.is_v4())
  {
    ...
    data_.v4.sin_addr.s_addr =
      ASIO_LIBNS::detail::socket_ops::host_to_network_long(
        addr.to_v4().to_uint());
  }
  else
  {
    ...
    data_.v6.sin6_port =
      ASIO_LIBNS::detail::socket_ops::host_to_network_short(port_num);
    data_.v6.sin6_flowinfo = 0;
    ASIO_LIBNS::ip::address_v6 v6_addr = addr.to_v6();
    ASIO_LIBNS::ip::address_v6::bytes_type bytes = v6_addr.to_bytes();
    memcpy(data_.v6.sin6_addr.s6_addr, bytes.data(), 16);
    data_.v6.sin6_scope_id =
      static_cast<ASIO_LIBNS::detail::u_long_type>(
        v6_addr.scope_id());
  }
}
...
{
  if (new_size > sizeof(ASIO_LIBNS::detail::sockaddr_storage_type))
  {
    ASIO_LIBNS::error_code ec(ASIO_LIBNS::error::invalid_argument);
...
unsigned short endpoint::port() const ASIO_NOEXCEPT
{
  if (is_v4())
  {
    return ASIO_LIBNS::detail::socket_ops::network_to_host_short(
        data_.v4.sin_port);
  }
  else
  {
...
ASIO_LIBNS::ip::address endpoint::address() const ASIO_NOEXCEPT
{
  using namespace std; // For memcpy.
  if (is_v4())
  {
    return ASIO_LIBNS::ip::address_v4(
        ASIO_LIBNS::detail::socket_ops::network_to_host_long(
          data_.v4.sin_addr.s_addr));
  }
  else
  {
    ASIO_LIBNS::ip::address_v6::bytes_type bytes;
#if defined(ASIO_HAS_STD_ARRAY)
    memcpy(bytes.data(), data_.v6.sin6_addr.s6_addr, 16);
#else // defined(ASIO_HAS_STD_ARRAY)
    memcpy(bytes.elems, data_.v6.sin6_addr.s6_addr, 16);
#endif // defined(ASIO_HAS_STD_ARRAY)
    return ASIO_LIBNS::ip::address_v6(bytes, data_.v6.sin6_scope_id);
  }
}

void endpoint::address(const ASIO_LIBNS::ip::address& addr) ASIO_NOEXCEPT
{
