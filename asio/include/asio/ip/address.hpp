...
/**
 * The ASIO_LIBNS::ip::address class provides the ability to use either IP
...
public:
  ...
  ASIO_DECL ASIO_LIBNS::ip::address_v4 to_v4() const;

  /// Get the address as an IP version 6 address.
  ASIO_DECL ASIO_LIBNS::ip::address_v6 to_v6() const;
  ...
  ASIO_DECL std::string to_string(ASIO_LIBNS::error_code& ec) const;
  ...
  /// string in dotted decimal form, or from an IPv6 address in hexadecimal
  /// notation.
  static address from_string(const char* str, ASIO_LIBNS::error_code& ec);
...
private:
  ...
  ASIO_LIBNS::ip::address_v4 ipv4_address_;

  // The underlying IPv6 address.
  ASIO_LIBNS::ip::address_v6 ipv6_address_;
};
...
template <>
struct hash<ASIO_LIBNS::ip::address>
{
  std::size_t operator()(const ASIO_LIBNS::ip::address& addr)
    const ASIO_NOEXCEPT
  {
    return addr.is_v4()
      ? std::hash<ASIO_LIBNS::ip::address_v4>()(addr.to_v4())
      : std::hash<ASIO_LIBNS::ip::address_v6>()(addr.to_v6());
  }
};
