...
public:
   ...
   * } @endcode
   */
  basic_yield_context operator[](ASIO_LIBNS::error_code& ec) const
  {
...
/**
 * @defgroup spawn ASIO_LIBNS::spawn
 *
 ...
 *
 * @code ASIO_LIBNS::spawn(my_strand, do_echo, ASIO_LIBNS::detached);
 *
 * // ...
 *
 * void do_echo(ASIO_LIBNS::yield_context yield)
 * {
 *   try
 *   {
 *     char data[128];
 ...
 *           ASIO_LIBNS::buffer(data), yield);
 *
 *       ASIO_LIBNS::async_write(my_socket,
 *           ASIO_LIBNS::buffer(data, length), yield);
 *     }
 *   }
 ...
 */
template <typename Function>
void spawn(const ASIO_LIBNS::io_context::strand& s,
    ASIO_MOVE_ARG(Function) function,
    const boost::coroutines::attributes& attributes
      = boost::coroutines::attributes());
