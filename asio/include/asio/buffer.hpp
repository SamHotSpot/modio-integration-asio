 ...
 * member functions:
 *
 * @code ASIO_LIBNS::mutable_buffer b1 = ...;
...
public:
  ...
  mutable_buffer(void* data, std::size_t size,
      ASIO_LIBNS::detail::function<void()> debug_check)
    : data_(data),
      size_(size),
  ...
  const ASIO_LIBNS::detail::function<void()>& get_debug_check() const
  {
    return debug_check_;
  }
#endif // ASIO_ENABLE_BUFFER_DEBUGGING
...
#if defined(ASIO_ENABLE_BUFFER_DEBUGGING)
  ASIO_LIBNS::detail::function<void()> debug_check_;
#endif // ASIO_ENABLE_BUFFER_DEBUGGING
};
 ...
 * member functions:
 *
 * @code ASIO_LIBNS::const_buffer b1 = ...;
 * std::size_t s1 = b1.size();
...
/** @defgroup buffer_sequence_begin ASIO_LIBNS::buffer_sequence_begin
 *
 * @brief The ASIO_LIBNS::buffer_sequence_begin function returns an iterator
...
/** @defgroup buffer_sequence_end ASIO_LIBNS::buffer_sequence_end
 *
 * @brief The ASIO_LIBNS::buffer_sequence_end function returns an iterator
 ...
 * auto i = ASIO_LIBNS::buffer_sequence_begin(buffers);
 * auto end = ASIO_LIBNS::buffer_sequence_end(buffers);
 * for (; i != end; ++i)
 * {
 *   const_buffer b(*i);
 *   total_size += b.size();
...
inline std::size_t buffer_size(const BufferSequence& b) ASIO_NOEXCEPT
{
  return detail::buffer_size(
      ...
      ASIO_LIBNS::buffer_sequence_begin(b),
      ASIO_LIBNS::buffer_sequence_end(b));
}

#if !defined(ASIO_NO_DEPRECATED)

/** @defgroup buffer_cast ASIO_LIBNS::buffer_cast
 *
 * @brief (Deprecated: Use the @c data() member function.) The
 * ASIO_LIBNS::buffer_cast function is used to obtain a pointer to the
 ...
 * @code ASIO_LIBNS::const_buffer b1 = ...;
 * const unsigned char* p1 = ASIO_LIBNS::buffer_cast<const unsigned char*>(b1);
 * @endcode
 *
 * To access the memory of a modifiable buffer, use:
 * @code ASIO_LIBNS::mutable_buffer b2 = ...;
 * unsigned char* p2 = ASIO_LIBNS::buffer_cast<unsigned char*>(b2);
 * @endcode
 *
 * The ASIO_LIBNS::buffer_cast function permits violations of type safety, so
...
/** @defgroup buffer ASIO_LIBNS::buffer
 *
 * @brief The ASIO_LIBNS::buffer function is used to create a buffer object to
 ...
 * specified size:
 *
 * @code sock.send(ASIO_LIBNS::buffer(data, size)); @endcode
 *
 * In the above example, the return value of ASIO_LIBNS::buffer meets the
 ...
 *
 * @code char d1[128];
 * size_t bytes_transferred = sock.receive(ASIO_LIBNS::buffer(d1));
 *
 * std::vector<char> d2(128);
 * bytes_transferred = sock.receive(ASIO_LIBNS::buffer(d2));
 *
 * std::array<char, 128> d3;
 * bytes_transferred = sock.receive(ASIO_LIBNS::buffer(d3));
 *
 * boost::array<char, 128> d4;
 * bytes_transferred = sock.receive(ASIO_LIBNS::buffer(d4)); @endcode
 *
 ...
 *
 * vector<unsigned char> data(ASIO_LIBNS::buffer_size(buffers));
 * ASIO_LIBNS::buffer_copy(ASIO_LIBNS::buffer(data), buffers); @endcode
 *
 ...
 *
 * For the ASIO_LIBNS::buffer overloads that accept an argument of type
 ...
 *
 * @code b1 = ASIO_LIBNS::buffer(a); @endcode
 *
 * represents the entire array, <tt>{ 'a', 'b', 'c', 'd', 'e' }</tt>. An
 * optional second argument to the ASIO_LIBNS::buffer function may be used to
 * limit the size, in bytes, of the buffer:
 *
 * @code b2 = ASIO_LIBNS::buffer(a, 3); @endcode
 *
 ...
 *
 * @code b4 = ASIO_LIBNS::buffer(b1 + 1, 3); @endcode
 *
 ...
 *   ASIO_LIBNS::buffer(d1),
 *   ASIO_LIBNS::buffer(d2),
 *   ASIO_LIBNS::buffer(d3) };
 * bytes_transferred = sock.receive(bufs1);
 *
 * std::vector<const_buffer> bufs2;
 * bufs2.push_back(ASIO_LIBNS::buffer(d1));
 * bufs2.push_back(ASIO_LIBNS::buffer(d2));
 * bufs2.push_back(ASIO_LIBNS::buffer(d3));
...
{
  typedef typename ASIO_LIBNS::detail::buffer_types<PodType>::buffer_type
    buffer_type;
  typedef typename ASIO_LIBNS::detail::buffer_types<PodType>::container_type
    container_type;
...
public:
  ...
  {
    return const_buffers_type(ASIO_LIBNS::buffer(string_, size_));
  }
#endif // !defined(ASIO_NO_DYNAMIC_BUFFER_V1)
  ...
  {
    return mutable_buffers_type(ASIO_LIBNS::buffer(
          ASIO_LIBNS::buffer(string_, max_size_) + pos, n));
  }
  ...
  {
    return const_buffers_type(ASIO_LIBNS::buffer(
          ASIO_LIBNS::buffer(string_, max_size_) + pos, n));
  }
  ...
  mutable_buffers_type prepare(std::size_t n)
  {
    ...
    return ASIO_LIBNS::buffer(ASIO_LIBNS::buffer(string_) + size_, n);
  }
...
public:
  ...
  const_buffers_type data() const ASIO_NOEXCEPT
  {
    return const_buffers_type(ASIO_LIBNS::buffer(vector_, size_));
  }
#endif // !defined(ASIO_NO_DYNAMIC_BUFFER_V1)
  ...
  {
    return mutable_buffers_type(ASIO_LIBNS::buffer(
          ASIO_LIBNS::buffer(vector_, max_size_) + pos, n));
  }
  ...
  mutable_buffers_type prepare(std::size_t n)
  {
    ...
    return ASIO_LIBNS::buffer(ASIO_LIBNS::buffer(vector_) + size_, n);
  }
...
/** @defgroup dynamic_buffer ASIO_LIBNS::dynamic_buffer
 *
 * @brief The ASIO_LIBNS::dynamic_buffer function is used to create a
...
/** @defgroup buffer_copy ASIO_LIBNS::buffer_copy
 *
 * @brief The ASIO_LIBNS::buffer_copy function is used to copy bytes from a
    ...
    std::size_t max_bytes_to_copy) ASIO_NOEXCEPT
{
  return (buffer_copy_1)(*target_begin,
      ASIO_LIBNS::buffer(*source_begin, max_bytes_to_copy));
}
      ...
      = (std::numeric_limits<std::size_t>::max)()) ASIO_NOEXCEPT
{
  ...
  for (mutable_buffer target_buffer(
        ASIO_LIBNS::buffer(*target_begin, max_bytes_to_copy));
    ...
    std::size_t max_bytes_to_copy) ASIO_NOEXCEPT
{
      ...
      && target_iter != target_end && source_iter != source_end)
  {
    ...
    std::size_t bytes_copied = (buffer_copy_1)(
        target_buffer, ASIO_LIBNS::buffer(source_buffer,
    ...
    const ConstBufferSequence& source) ASIO_NOEXCEPT
{
  return detail::buffer_copy(
      ...
      ASIO_LIBNS::buffer_sequence_begin(target),
      ASIO_LIBNS::buffer_sequence_end(target),
      ASIO_LIBNS::buffer_sequence_begin(source),
      ASIO_LIBNS::buffer_sequence_end(source));
}
    ...
    std::size_t max_bytes_to_copy) ASIO_NOEXCEPT
{
  return detail::buffer_copy(
      ...
      ASIO_LIBNS::buffer_sequence_end(target),
      ASIO_LIBNS::buffer_sequence_begin(source),
      ASIO_LIBNS::buffer_sequence_end(source), max_bytes_to_copy);
}
...
struct is_mutable_buffer_sequence
  ...
  : ASIO_LIBNS::detail::is_buffer_sequence<T, mutable_buffer>
...
struct is_const_buffer_sequence
  ...
  : integral_constant<bool, automatically_determined>
#else // defined(GENERATING_DOCUMENTATION)
  : ASIO_LIBNS::detail::is_buffer_sequence<T, const_buffer>
#endif // defined(GENERATING_DOCUMENTATION)
{
};
...
struct is_dynamic_buffer_v1
  ...
  : integral_constant<bool, automatically_determined>
#else // defined(GENERATING_DOCUMENTATION)
  : ASIO_LIBNS::detail::is_dynamic_buffer_v1<T>
#endif // defined(GENERATING_DOCUMENTATION)
{
};
#endif // !defined(ASIO_NO_DYNAMIC_BUFFER_V1)
...
struct is_dynamic_buffer_v2
  ...
  : integral_constant<bool, automatically_determined>
#else // defined(GENERATING_DOCUMENTATION)
  : ASIO_LIBNS::detail::is_dynamic_buffer_v2<T>
#endif // defined(GENERATING_DOCUMENTATION)
{
};
...
struct is_dynamic_buffer
  ...
  : ASIO_LIBNS::is_dynamic_buffer_v2<T>
#else // defined(ASIO_NO_DYNAMIC_BUFFER_V1)
  : ASIO_LIBNS::is_dynamic_buffer_v1<T>
#endif // defined(ASIO_NO_DYNAMIC_BUFFER_V1)
{
};
