...
public:
  ...
  void send_break()
  {
    ...
    ASIO_LIBNS::detail::throw_error(ec, "send_break");
  }
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  ASIO_SYNC_OP_VOID send_break(ASIO_LIBNS::error_code& ec)
  {
    impl_.get_service().send_break(impl_.get_implementation(), ec);
    ASIO_SYNC_OP_VOID_RETURN(ec);
  }
   ...
   * ASIO_LIBNS::serial_port_base::baud_rate @n
   * ASIO_LIBNS::serial_port_base::flow_control @n
   * ASIO_LIBNS::serial_port_base::parity @n
   * ASIO_LIBNS::serial_port_base::stop_bits @n
   * ASIO_LIBNS::serial_port_base::character_size
   */
   ...
   * @code
   * basic_serial_port.write_some(ASIO_LIBNS::buffer(data, size));
   * @endcode
   ...
   *     ASIO_LIBNS::buffer(data, size), handler);
   * @endcode
   * See the @ref buffer documentation for information on writing multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
   ...
   * @code
   * basic_serial_port.read_some(ASIO_LIBNS::buffer(data, size));
   * @endcode
