...
public:
  ...
  uint64_t seek(int64_t offset, file_base::seek_basis whence)
  {
    ...
    ASIO_LIBNS::detail::throw_error(ec, "seek");
    return n;
  }
   ...
   * @code
   * file.write_some(ASIO_LIBNS::buffer(data, size));
   * @endcode
   ...
   * To write a single data buffer use the @ref buffer function as follows:
   * @code
   * file.async_write_some(ASIO_LIBNS::buffer(data, size), handler);
   * @endcode
   * See the @ref buffer documentation for information on writing multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
   ...
   * @code
   * file.read_some(ASIO_LIBNS::buffer(data, size));
   * @endcode
   ...
   * To read into a single data buffer use the @ref buffer function as follows:
   * @code
   * file.async_read_some(ASIO_LIBNS::buffer(data, size), handler);
   * @endcode
   * See the @ref buffer documentation for information on reading into multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
