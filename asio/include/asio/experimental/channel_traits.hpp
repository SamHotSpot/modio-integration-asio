...
struct channel_traits<R(ASIO_LIBNS::error_code, Args...), Signatures...>
{
  template <typename... NewSignatures>
  struct rebind
  {
  ...
  typedef R receive_cancelled_signature(ASIO_LIBNS::error_code, Args...);

  template <typename F>
  static void invoke_receive_cancelled(F f)
  {
    const ASIO_LIBNS::error_code e = error::channel_cancelled;
    ASIO_MOVE_OR_LVALUE(F)(f)(e, typename decay<Args>::type()...);
  }

  typedef R receive_closed_signature(ASIO_LIBNS::error_code, Args...);

  template <typename F>
  static void invoke_receive_closed(F f)
  {
    const ASIO_LIBNS::error_code e = error::channel_closed;
    ASIO_MOVE_OR_LVALUE(F)(f)(e, typename decay<Args>::type()...);
  }
};
...
struct channel_traits<R(std::exception_ptr, Args...), Signatures...>
{
  ...
  static void invoke_receive_cancelled(F f)
  {
    ...
    ASIO_MOVE_OR_LVALUE(F)(f)(
        std::make_exception_ptr(ASIO_LIBNS::system_error(e)),
...
struct channel_traits<R()>
{
  ...
  typedef R receive_cancelled_signature(ASIO_LIBNS::error_code);
  ...
  typedef R receive_closed_signature(ASIO_LIBNS::error_code);
