...
inline channel_service<Mutex>::channel_service(execution_context& ctx)
  : ASIO_LIBNS::detail::execution_context_service_base<channel_service>(ctx),
    mutex_(),
    impl_list_(0)
{
}
...
inline void channel_service<Mutex>::shutdown()
{
  ...
  ASIO_LIBNS::detail::op_queue<channel_operation> ops;
    ...
    void* cancellation_key)
{
  ...
  ASIO_LIBNS::detail::op_queue<channel_operation> other_ops;
  while (channel_operation* op = impl.waiters_.front())
  {
  ...
  case buffer:
    {
      ...
      lock.unlock();
      ASIO_LIBNS::detail::non_const_lvalue<Handler> handler2(handler);
