...
{
  typedef ASIO_LIBNS::detail::array<std::size_t, N> order_type;
...
void parallel_group_launch(Condition cancellation_condition, Handler handler,
    std::tuple<Ops...>& ops, ASIO_LIBNS::detail::index_sequence<I...>);

} // namespace detail

/// A group of asynchronous operations that may be launched in parallel.
/**
 * See the documentation for ASIO_LIBNS::experimental::make_parallel_group for
 * a usage example.
 */
  ...
  struct initiate_async_wait
  {
    ...
    void operator()(Handler&& h, Condition&& c, std::tuple<Ops...>&& ops) const
    {
      detail::parallel_group_launch(
          ...
          ops, ASIO_LIBNS::detail::index_sequence_for<Ops...>());
    }
  };
...
public:
   ...
   * operations, it must return a ASIO_LIBNS::cancellation_type value other
   * than <tt>ASIO_LIBNS::cancellation_type::none</tt>.
   *
   ...
   *
   * @li ASIO_LIBNS::experimental::wait_for_all
   * @li ASIO_LIBNS::experimental::wait_for_one
   * @li ASIO_LIBNS::experimental::wait_for_one_error
   * @li ASIO_LIBNS::experimental::wait_for_one_success
   */
  ...
  async_wait(CancellationCondition cancellation_condition,
    ...
    ASIO_INITFN_AUTO_RESULT_TYPE_SUFFIX((
      ASIO_LIBNS::async_initiate<CompletionToken, signature>(
          declval<initiate_async_wait>(), token,
          std::move(cancellation_condition), std::move(ops_))))
  {
    return ASIO_LIBNS::async_initiate<CompletionToken, signature>(
...
/**
 * For example:
 * @code ASIO_LIBNS::experimental::make_parallel_group(
 *    [&](auto token)
 *    {
 *      return in.async_read_some(ASIO_LIBNS::buffer(data), token);
 *    },
 *    [&](auto token)
 *    {
 ...
 *    }
 *  ).async_wait(
 *    ASIO_LIBNS::experimental::wait_for_all(),
 *    [](
