...
struct parallel_group_completion_handler
{
  ...
  parallel_group_completion_handler(Handler&& h)
    : handler_(std::move(h)),
      executor_(
          ASIO_LIBNS::prefer(
            ASIO_LIBNS::get_associated_executor(handler_),
  ...
  void operator()()
  {
    this->invoke(ASIO_LIBNS::detail::make_index_sequence<sizeof...(Ops)>());
  }

  template <std::size_t... I>
  void invoke(ASIO_LIBNS::detail::index_sequence<I...>)
  {
  ...
  void invoke(std::tuple<Args...>&& args)
  {
    this->invoke(std::move(args),
        ASIO_LIBNS::detail::index_sequence_for<Args...>());
  }

  template <typename... Args, std::size_t... I>
  void invoke(std::tuple<Args...>&& args,
      ASIO_LIBNS::detail::index_sequence<I...>)
  {
...
struct parallel_group_state
{
  ...
  ASIO_LIBNS::cancellation_signal cancellation_signals_[sizeof...(Ops)];
...
{
  typedef ASIO_LIBNS::cancellation_slot cancellation_slot_type;
  ...
  void operator()(Args... args)
  {
    ...
    if (--state_->outstanding_ == 0)
      ASIO_LIBNS::dispatch(std::move(state_->handler_));
  }
  ...
  parallel_group_op_handler<I, Condition, Handler, Ops...>
{
  ...
  struct cancel_proxy
  {
    ...
    void operator()(cancellation_type_t type)
    {
      ...
      {
        ASIO_LIBNS::cancellation_signal* sig = &signal_;
        ASIO_LIBNS::dispatch(executor_,
            [state, sig, type]{ sig->emit(type); });
      }
    }

    std::weak_ptr<parallel_group_state<Condition, Handler, Ops...> > state_;
    ASIO_LIBNS::cancellation_signal signal_;
    executor_type executor_;
  };
...
struct parallel_group_op_launcher
{
      ...
      Condition, Handler, Ops...> >& state)
  {
    ...
    ex_type ex = ASIO_LIBNS::get_associated_executor(op);
    std::move(op)(
    ...
    std::tuple<Ops...>& ops, ASIO_LIBNS::detail::index_sequence<I...>)
{
  ...
  std::shared_ptr<state_type> state = std::allocate_shared<state_type>(
      ASIO_LIBNS::detail::recycling_allocator<state_type,
        ASIO_LIBNS::detail::thread_info_base::parallel_group_tag>(),
