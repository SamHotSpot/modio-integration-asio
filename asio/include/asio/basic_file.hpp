...
public:
   ...
   * @par Example
   * @code
   * ASIO_LIBNS::stream_file file(my_context);
   * file.open("/path/to/my/file", ASIO_LIBNS::stream_file::read_only);
   * @endcode
   */
   ...
   * ASIO_LIBNS::stream_file file(my_context);
   * ASIO_LIBNS::error_code ec;
   * file.open("/path/to/my/file", ASIO_LIBNS::stream_file::read_only, ec);
   * if (ec)
   * {
   *   // An error occurred.
   * }
  ...
  ASIO_SYNC_OP_VOID open(const char* path,
      file_base::flags open_flags, ASIO_LIBNS::error_code& ec)
  {
    impl_.get_service().open(impl_.get_implementation(), path, open_flags, ec);
   ...
   * operations will be passed the ASIO_LIBNS::error::operation_aborted error.
  ...
  uint64_t size() const
  {
    ...
    ASIO_LIBNS::detail::throw_error(ec, "size");
    return s;
  }
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  uint64_t size(ASIO_LIBNS::error_code& ec) const
  {
  ...
  void resize(uint64_t n)
  {
    ...
    ASIO_LIBNS::detail::throw_error(ec, "resize");
  }
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  ASIO_SYNC_OP_VOID resize(uint64_t n, ASIO_LIBNS::error_code& ec)
  {
    impl_.get_service().resize(impl_.get_implementation(), n, ec);
    ASIO_SYNC_OP_VOID_RETURN(ec);
  }
  ...
  void sync_all()
  {
    ...
    ASIO_LIBNS::detail::throw_error(ec, "sync_all");
  }
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  ASIO_SYNC_OP_VOID sync_all(ASIO_LIBNS::error_code& ec)
  {
    impl_.get_service().sync_all(impl_.get_implementation(), ec);
    ASIO_SYNC_OP_VOID_RETURN(ec);
  }
  ...
  void sync_data()
  {
    ...
    ASIO_LIBNS::detail::throw_error(ec, "sync_data");
  }
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  ASIO_SYNC_OP_VOID sync_data(ASIO_LIBNS::error_code& ec)
  {
    impl_.get_service().sync_data(impl_.get_implementation(), ec);
    ASIO_SYNC_OP_VOID_RETURN(ec);
  }
