...
public:
  ...
  ASIO_DECL win_iocp_io_context(ASIO_LIBNS::execution_context& ctx,
  ...
  ASIO_DECL ASIO_LIBNS::error_code register_handle(
      HANDLE handle, ASIO_LIBNS::error_code& ec);

  // Run the event loop until stopped or no more work.
  ASIO_DECL size_t run(ASIO_LIBNS::error_code& ec);

  // Run until stopped or one operation is performed.
  ASIO_DECL size_t run_one(ASIO_LIBNS::error_code& ec);

  // Run until timeout, interrupted, or one operation is performed.
  ASIO_DECL size_t wait_one(long usec, ASIO_LIBNS::error_code& ec);

  // Poll for operations without blocking.
  ASIO_DECL size_t poll(ASIO_LIBNS::error_code& ec);

  // Poll for one operation without blocking.
  ASIO_DECL size_t poll_one(ASIO_LIBNS::error_code& ec);
  ...
  // starting the operation.
  ASIO_DECL void on_completion(win_iocp_operation* op,
      const ASIO_LIBNS::error_code& ec, DWORD bytes_transferred = 0);
