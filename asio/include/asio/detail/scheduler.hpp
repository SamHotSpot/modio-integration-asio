...
public:
  ...
  typedef scheduler_task* (*get_task_func_type)(
      ASIO_LIBNS::execution_context&);

  // Constructor. Specifies the number of concurrent threads that are likely to
  // run the scheduler. If set to 1 certain optimisation are performed.
  ASIO_DECL scheduler(ASIO_LIBNS::execution_context& ctx,
  ...
  ASIO_DECL std::size_t run(ASIO_LIBNS::error_code& ec);

  // Run until interrupted or one operation is performed.
  ASIO_DECL std::size_t run_one(ASIO_LIBNS::error_code& ec);

  // Run until timeout, interrupted, or one operation is performed.
  ASIO_DECL std::size_t wait_one(
      long usec, ASIO_LIBNS::error_code& ec);

  // Poll for operations without blocking.
  ASIO_DECL std::size_t poll(ASIO_LIBNS::error_code& ec);

  // Poll for one operation without blocking.
  ASIO_DECL std::size_t poll_one(ASIO_LIBNS::error_code& ec);
...
private:
  ...
  ASIO_DECL std::size_t do_run_one(mutex::scoped_lock& lock,
      thread_info& this_thread, const ASIO_LIBNS::error_code& ec);

  // Run at most one operation with a timeout. May block.
  ASIO_DECL std::size_t do_wait_one(mutex::scoped_lock& lock,
      thread_info& this_thread, long usec, const ASIO_LIBNS::error_code& ec);
  ...
  ASIO_DECL static scheduler_task* get_default_task(
      ASIO_LIBNS::execution_context& ctx);
