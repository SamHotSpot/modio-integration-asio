...
inline void get_last_error(
    ASIO_LIBNS::error_code& ec, bool is_error_condition)
{
  if (!is_error_condition)
  {
    ASIO_LIBNS::error::clear(ec);
  }
  else
  {
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
    ec = ASIO_LIBNS::error_code(WSAGetLastError(),
        ASIO_LIBNS::error::get_system_category());
#else
    ec = ASIO_LIBNS::error_code(errno,
        ASIO_LIBNS::error::get_system_category());
#endif
  }
}
...
socket_type accept(socket_type s, void* addr,
    std::size_t* addrlen, ASIO_LIBNS::error_code& ec)
{
  if (s == invalid_socket)
  {
...
socket_type sync_accept(socket_type s, state_type state,
    void* addr, std::size_t* addrlen, ASIO_LIBNS::error_code& ec)
{
  // Accept a socket.
  for (;;)
  {
    ...
    else if (ec == ASIO_LIBNS::error::connection_aborted)
    {
...
void complete_iocp_accept(socket_type s, void* output_buffer,
    ...
    socket_type new_socket, ASIO_LIBNS::error_code& ec)
{
  // Map non-portable errors to their portable counterparts.
  if (ec.value() == ERROR_NETNAME_DELETED)
    ec = ASIO_LIBNS::error::connection_aborted;
  ...
  if (!ec)
  {
    ...
    if (addr && addrlen)
    {
      ...
      {
        ec = ASIO_LIBNS::error::invalid_argument;
      }
      else
      {
...
bool non_blocking_accept(socket_type s,
    ...
    ASIO_LIBNS::error_code& ec, socket_type& new_socket)
{
  for (;;)
  {
...
int close(socket_type s, state_type& state,
    bool destruction, ASIO_LIBNS::error_code& ec)
{
  int result = 0;
  if (s != invalid_socket)
  {
    ...
    if (result != 0
        && (ec == ASIO_LIBNS::error::would_block
          || ec == ASIO_LIBNS::error::try_again))
    {
...
bool set_user_non_blocking(socket_type s,
    state_type& state, bool value, ASIO_LIBNS::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = ASIO_LIBNS::error::bad_descriptor;
...
int shutdown(socket_type s, int what, ASIO_LIBNS::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = ASIO_LIBNS::error::bad_descriptor;
    ...
    std::size_t addrlen, ASIO_LIBNS::error_code& ec)
{
  ...
  if (result != 0 && ec == ASIO_LIBNS::error::try_again)
  {
    if (static_cast<const socket_addr_type*>(addr)->sa_family == AF_UNIX)
      ec = ASIO_LIBNS::error::in_progress;
    else
      ec = ASIO_LIBNS::error::no_buffer_space;
  }
    ...
    std::size_t addrlen, ASIO_LIBNS::error_code& ec)
{
  ...
  if (ec != ASIO_LIBNS::error::in_progress
      && ec != ASIO_LIBNS::error::would_block)
  {
  ...
  ec = ASIO_LIBNS::error_code(connect_error,
      ASIO_LIBNS::error::get_system_category());
}

#if defined(ASIO_HAS_IOCP)

void complete_iocp_connect(socket_type s, ASIO_LIBNS::error_code& ec)
{
  // Map non-portable errors to their portable counterparts.
  switch (ec.value())
  {
  ...
  case ERROR_NETWORK_UNREACHABLE:
    ec = ASIO_LIBNS::error::network_unreachable;
    break;
  case ERROR_HOST_UNREACHABLE:
    ec = ASIO_LIBNS::error::host_unreachable;
    break;
  case ERROR_SEM_TIMEOUT:
    ec = ASIO_LIBNS::error::timed_out;
    break;
  default:
    break;
  }
...
bool non_blocking_connect(socket_type s, ASIO_LIBNS::error_code& ec)
{
...
int socketpair(int af, int type, int protocol,
    socket_type sv[2], ASIO_LIBNS::error_code& ec)
{
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(af);
  (void)(type);
...
bool sockatmark(socket_type s, ASIO_LIBNS::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = ASIO_LIBNS::error::bad_descriptor;
...
# if defined(ENOTTY)
  if (ec.value() == ENOTTY)
    ec = ASIO_LIBNS::error::not_socket;
...
size_t available(socket_type s, ASIO_LIBNS::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = ASIO_LIBNS::error::bad_descriptor;
...
int listen(socket_type s, int backlog, ASIO_LIBNS::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = ASIO_LIBNS::error::bad_descriptor;
...
signed_size_type recv(socket_type s, buf* bufs, size_t count,
    int flags, ASIO_LIBNS::error_code& ec)
{
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  // Receive some data.
...
size_t sync_recv(socket_type s, state_type state, buf* bufs,
    size_t count, int flags, bool all_empty, ASIO_LIBNS::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = ASIO_LIBNS::error::bad_descriptor;
...
size_t sync_recv1(socket_type s, state_type state, void* data,
    size_t size, int flags, ASIO_LIBNS::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = ASIO_LIBNS::error::bad_descriptor;
...
void complete_iocp_recv(state_type state,
    ...
    ASIO_LIBNS::error_code& ec, size_t bytes_transferred)
{
  // Map non-portable errors to their portable counterparts.
  if (ec.value() == ERROR_NETNAME_DELETED)
  {
...
signed_size_type recvfrom(socket_type s, buf* bufs, size_t count,
    int flags, void* addr, std::size_t* addrlen, ASIO_LIBNS::error_code& ec)
{
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  // Receive some data.
  DWORD recv_buf_count = static_cast<DWORD>(count);
...
signed_size_type recvmsg(socket_type s, buf* bufs, size_t count,
    int in_flags, int& out_flags, ASIO_LIBNS::error_code& ec)
{
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  out_flags = 0;
...
int setsockopt(socket_type s, state_type& state, int level, int optname,
    const void* optval, std::size_t optlen, ASIO_LIBNS::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = ASIO_LIBNS::error::bad_descriptor;
...
int getsockopt(socket_type s, state_type state, int level, int optname,
    void* optval, size_t* optlen, ASIO_LIBNS::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = ASIO_LIBNS::error::bad_descriptor;
...
int getpeername(socket_type s, void* addr, std::size_t* addrlen,
    bool cached, ASIO_LIBNS::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = ASIO_LIBNS::error::bad_descriptor;
  ...
  if (cached)
  {
    ...
    {
      ec = ASIO_LIBNS::error::not_connected;
      return socket_error_retval;
    }
...
int ioctl(socket_type s, state_type& state, int cmd,
    ioctl_arg_type* arg, ASIO_LIBNS::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = ASIO_LIBNS::error::bad_descriptor;
...
int select(int nfds, fd_set* readfds, fd_set* writefds,
    fd_set* exceptfds, timeval* timeout, ASIO_LIBNS::error_code& ec)
{
...
int poll_read(socket_type s, state_type state,
    int msec, ASIO_LIBNS::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = ASIO_LIBNS::error::bad_descriptor;
...
int poll_connect(socket_type s, int msec, ASIO_LIBNS::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = ASIO_LIBNS::error::bad_descriptor;
...
const char* inet_ntop(int af, const void* src, char* dest, size_t length,
    unsigned long scope_id, ASIO_LIBNS::error_code& ec)
{
  clear_last_error();
#if defined(ASIO_WINDOWS_RUNTIME)
  ...
  {
    ec = ASIO_LIBNS::error::address_family_not_supported;
    return 0;
  }
#elif defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  using namespace std; // For memcpy.
...
int gethostname(char* name, int namelen, ASIO_LIBNS::error_code& ec)
{
#if defined(ASIO_WINDOWS_RUNTIME)
  try
  {
  ...
  {
    ec = ASIO_LIBNS::error_code(e.code(),
...
inline ASIO_LIBNS::error_code translate_netdb_error(int error)
{
  switch (error)
  {
  case 0:
  ...
  case HOST_NOT_FOUND:
    return ASIO_LIBNS::error::host_not_found;
  case TRY_AGAIN:
    return ASIO_LIBNS::error::host_not_found_try_again;
  case NO_RECOVERY:
    return ASIO_LIBNS::error::no_recovery;
  case NO_DATA:
    return ASIO_LIBNS::error::no_data;
  default:
    ASIO_ASSERT(false);
    return ASIO_LIBNS::error::invalid_argument;
  }
}

inline hostent* gethostbyaddr(const char* addr, int length, int af,
    hostent* result, char* buffer, int buflength, ASIO_LIBNS::error_code& ec)
{
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
  (void)(buflength);
...
inline hostent* gethostbyname(const char* name, int af, struct hostent* result,
    char* buffer, int buflength, int ai_flags, ASIO_LIBNS::error_code& ec)
{
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
  (void)(buflength);
    ...
    const addrinfo_type* hintsp, addrinfo_type** result)
{
  ...
  for (gai_search* sptr = search; sptr < search + search_count; ++sptr)
  {
    ...
    if (hptr == 0)
    {
      ...
      freeaddrinfo_emulation(aihead);
      gai_free(canon);
      if (ec == ASIO_LIBNS::error::host_not_found)
        return EAI_NONAME;
      if (ec == ASIO_LIBNS::error::host_not_found_try_again)
        return EAI_AGAIN;
      if (ec == ASIO_LIBNS::error::no_recovery)
        return EAI_FAIL;
      if (ec == ASIO_LIBNS::error::no_data)
        return EAI_NONAME;
      return EAI_NONAME;
    }
...
inline ASIO_LIBNS::error_code getnameinfo_emulation(
    ...
    ASIO_LIBNS::error_code& ec)
{
  ...
  case ASIO_OS_DEF(AF_INET):
    ...
    {
      return ec = ASIO_LIBNS::error::invalid_argument;
    }
  ...
  case ASIO_OS_DEF(AF_INET6):
    ...
    break;
  default:
    return ec = ASIO_LIBNS::error::address_family_not_supported;
  }
  ...
  if (host && hostlen > 0)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          return ec = ASIO_LIBNS::error::host_not_found;
        }
  ...
  if (serv && servlen > 0)
  {
    if (flags & NI_NUMERICSERV)
    {
      if (servlen < 6)
      {
        return ec = ASIO_LIBNS::error::no_buffer_space;
      }
#if defined(ASIO_HAS_SECURE_RTL)
...
inline ASIO_LIBNS::error_code translate_addrinfo_error(int error)
{
  switch (error)
  {
  case 0:
  ...
  case EAI_FAIL:
    return ASIO_LIBNS::error::no_recovery;
  case EAI_FAMILY:
    return ASIO_LIBNS::error::address_family_not_supported;
  case EAI_MEMORY:
    return ASIO_LIBNS::error::no_memory;
  case EAI_NONAME:
  ...
  case EAI_NODATA:
    ...
    return ASIO_LIBNS::error::host_not_found;
  case EAI_SERVICE:
    return ASIO_LIBNS::error::service_not_found;
  case EAI_SOCKTYPE:
    return ASIO_LIBNS::error::socket_type_not_supported;
  default: // Possibly the non-portable EAI_SYSTEM.
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
    return ASIO_LIBNS::error_code(
        WSAGetLastError(), ASIO_LIBNS::error::get_system_category());
#else
    return ASIO_LIBNS::error_code(
        errno, ASIO_LIBNS::error::get_system_category());
#endif
  }
}

ASIO_LIBNS::error_code getaddrinfo(const char* host,
    const char* service, const addrinfo_type& hints,
    addrinfo_type** result, ASIO_LIBNS::error_code& ec)
{
...
ASIO_LIBNS::error_code background_getaddrinfo(
...
ASIO_LIBNS::error_code getnameinfo(const void* addr,
    std::size_t addrlen, char* host, std::size_t hostlen,
    char* serv, std::size_t servlen, int flags, ASIO_LIBNS::error_code& ec)
{
#if defined(ASIO_WINDOWS) || defined(__CYGWIN__)
# if defined(ASIO_HAS_GETADDRINFO)
  // Building for Windows XP, Windows Server 2003, or later.
...
ASIO_LIBNS::error_code sync_getnameinfo(const void* addr,
    std::size_t addrlen, char* host, std::size_t hostlen, char* serv,
    std::size_t servlen, int sock_type, ASIO_LIBNS::error_code& ec)
{
...
ASIO_LIBNS::error_code background_getnameinfo(
    const weak_cancel_token_type& cancel_token,
    const void* addr, std::size_t addrlen,
