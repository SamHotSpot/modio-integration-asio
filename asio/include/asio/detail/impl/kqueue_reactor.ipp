...
kqueue_reactor::kqueue_reactor(ASIO_LIBNS::execution_context& ctx)
    ...
    registered_descriptors_mutex_(mutex_.enabled())
{
  ...
  {
    ASIO_LIBNS::error_code error(errno,
        ASIO_LIBNS::error::get_system_category());
    ASIO_LIBNS::detail::throw_error(error);
  }
}
  ...
  if (fork_ev == ASIO_LIBNS::execution_context::fork_child)
  {
    ...
    if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1)
    {
      ASIO_LIBNS::error_code ec(errno,
          ASIO_LIBNS::error::get_system_category());
      ASIO_LIBNS::detail::throw_error(ec, "kqueue interrupter registration");
    }
        ...
        state != 0; state = state->next_)
    {
      if (state->num_kevents_ > 0)
      {
        ...
        if (::kevent(kqueue_fd_, events, state->num_kevents_, 0, 0, 0) == -1)
        {
          ASIO_LIBNS::error_code ec(errno,
              ASIO_LIBNS::error::get_system_category());
          ASIO_LIBNS::detail::throw_error(ec, "kqueue re-registration");
        }
      }
    }
  }
  ...
  if (descriptor_data->op_queue_[op_type].empty())
  {
          ...
          || descriptor_data->op_queue_[except_op].empty()))
    {
      ...
      if (descriptor_data->num_kevents_ < num_kevents[op_type])
      {
        ...
        {
          op->ec_ = ASIO_LIBNS::error_code(errno,
              ASIO_LIBNS::error::get_system_category());
          scheduler_.post_immediate_completion(op, is_continuation);
          return;
        }
...
int kqueue_reactor::do_kqueue_create()
{
  ...
  if (fd == -1)
  {
    ASIO_LIBNS::error_code ec(errno,
        ASIO_LIBNS::error::get_system_category());
    ASIO_LIBNS::detail::throw_error(ec, "kqueue");
  }
  return fd;
}
