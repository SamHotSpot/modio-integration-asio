...
select_reactor::select_reactor(ASIO_LIBNS::execution_context& ctx)
    ...
    ASIO_LIBNS::execution_context::fork_event fork_ev)
{
  ...
  (void)fork_ev;
#else // defined(ASIO_HAS_IOCP)
  if (fork_ev == ASIO_LIBNS::execution_context::fork_child)
...
{
  ASIO_LIBNS::detail::mutex::scoped_lock lock(mutex_);
  cancel_ops_unlocked(descriptor, ASIO_LIBNS::error::operation_aborted);
}
    ...
    int op_type, void* cancellation_key)
{
  ...
  bool need_interrupt = op_queue_[op_type].cancel_operations_by_key(
      descriptor, ops, cancellation_key, ASIO_LIBNS::error::operation_aborted);
    ...
    const ASIO_LIBNS::error_code& /*ec*/, std::size_t /*bytes_transferred*/)
{
  if (owner)
  {
    ...
    ASIO_LIBNS::detail::mutex::scoped_lock lock(reactor->mutex_);
    ...
    ASIO_LIBNS::detail::signal_blocker sb;
    reactor->thread_ =
      new ASIO_LIBNS::detail::thread(thread_function(reactor));
  }
}
#endif // defined(ASIO_HAS_IOCP)
