...
ASIO_LIBNS::error_code win_iocp_socket_service_base::close(
    win_iocp_socket_service_base::base_implementation_type& impl,
...
ASIO_LIBNS::error_code win_iocp_socket_service_base::cancel(
    win_iocp_socket_service_base::base_implementation_type& impl,
    ASIO_LIBNS::error_code& ec)
{
  if (!is_open(impl))
...
ASIO_LIBNS::error_code win_iocp_socket_service_base::do_open(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int family, int type, int protocol, ASIO_LIBNS::error_code& ec)
{
  if (is_open(impl))
  {
...
ASIO_LIBNS::error_code win_iocp_socket_service_base::do_assign(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int type, socket_type native_socket, ASIO_LIBNS::error_code& ec)
{
  if (is_open(impl))
  {
    ec = ASIO_LIBNS::error::already_open;
    ...
    socket_base::message_flags flags, bool noop, operation* op)
{
  ...
  if (noop)
    ...
    iocp_service_.on_completion(op, ASIO_LIBNS::error::bad_descriptor);
  else
  {
    ...
    int protocol, void* output_buffer, DWORD address_length, operation* op)
{
  ...
  if (!is_open(impl))
    ...
    iocp_service_.on_completion(op, ASIO_LIBNS::error::already_open);
  else
  {
    ASIO_LIBNS::error_code ec;
    ...
    long* cancel_requested, operation* op)
{
  ...
  if (cancel_requested)
    if (::InterlockedExchangeAdd(cancel_requested, 0) == 1)
      iocp_service_.on_completion(op, ASIO_LIBNS::error::operation_aborted);
    ...
    win_iocp_socket_connect_op_base* op, operation* iocp_op)
{
      ...
      || family == ASIO_OS_DEF(AF_INET6))
  {
    if (connect_ex_fn connect_ex = get_connect_ex(impl, type))
    {
      ...
      if (op->ec_ && op->ec_ != ASIO_LIBNS::error::invalid_argument)
      {
        iocp_service_.post_immediate_completion(op, false);
        return -1;
      }
        ...
        impl.socket_, impl.state_, true, op->ec_))
  {
    ...
    {
      if (op->ec_ == ASIO_LIBNS::error::in_progress
          || op->ec_ == ASIO_LIBNS::error::would_block)
      {
