...
win_iocp_io_context::win_iocp_io_context(
    ASIO_LIBNS::execution_context& ctx, int concurrency_hint, bool own_thread)
    ...
    concurrency_hint_(concurrency_hint)
{
  ...
  if (!iocp_.handle)
  {
    ...
    ASIO_LIBNS::detail::throw_error(ec, "iocp");
  }

  if (own_thread)
  {
    ::InterlockedIncrement(&outstanding_work_);
    thread_.reset(new ASIO_LIBNS::detail::thread(thread_function(this)));
  }
}
...
ASIO_LIBNS::error_code win_iocp_io_context::register_handle(
    HANDLE handle, ASIO_LIBNS::error_code& ec)
{
  if (::CreateIoCompletionPort(handle, iocp_.handle, 0, 0) == 0)
  {
    DWORD last_error = ::GetLastError();
    ec = ASIO_LIBNS::error_code(last_error,
        ASIO_LIBNS::error::get_system_category());
  }
  else
  {
...
size_t win_iocp_io_context::run(ASIO_LIBNS::error_code& ec)
{
...
size_t win_iocp_io_context::run_one(ASIO_LIBNS::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::wait_one(long usec, ASIO_LIBNS::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::poll(ASIO_LIBNS::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::poll_one(ASIO_LIBNS::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
  ...
  if (::InterlockedExchange(&stopped_, 1) == 0)
  {
    if (::InterlockedExchange(&stop_event_posted_, 1) == 0)
    {
      if (!::PostQueuedCompletionStatus(iocp_.handle, 0, 0, 0))
      {
        ...
        ASIO_LIBNS::error_code ec(last_error,
            ASIO_LIBNS::error::get_system_category());
        ASIO_LIBNS::detail::throw_error(ec, "pqcs");
      }
    }
  }
}
...
void win_iocp_io_context::on_completion(win_iocp_operation* op,
    const ASIO_LIBNS::error_code& ec, DWORD bytes_transferred)
{
  // Flag that the operation is ready for invocation.
  op->ready_ = 1;
...
size_t win_iocp_io_context::do_one(DWORD msec,
    win_iocp_thread_info& this_thread, ASIO_LIBNS::error_code& ec)
{
  for (;;)
  {
    ...
    if (overlapped)
    {
      ...
      ASIO_LIBNS::error_code result_ec(last_error,
          ASIO_LIBNS::error::get_system_category());
      ...
      {
        result_ec = ASIO_LIBNS::error_code(static_cast<int>(op->Offset),
            *reinterpret_cast<ASIO_LIBNS::error_category*>(op->Internal));
...
void win_iocp_io_context::do_add_timer_queue(timer_queue_base& queue)
{
  ...
  if (!waitable_timer_.handle)
  {
    ...
    if (waitable_timer_.handle == 0)
    {
      ...
      ASIO_LIBNS::error_code ec(last_error,
          ASIO_LIBNS::error::get_system_category());
      ASIO_LIBNS::detail::throw_error(ec, "timer");
    }
