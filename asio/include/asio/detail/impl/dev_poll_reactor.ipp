...
dev_poll_reactor::dev_poll_reactor(ASIO_LIBNS::execution_context& ctx)
  : ASIO_LIBNS::detail::execution_context_service_base<dev_poll_reactor>(ctx),
...
void dev_poll_reactor::run(long usec, op_queue<operation>& ops)
{
  ...
  if (events_size > 0)
  {
    ...
    {
      ASIO_LIBNS::error_code ec = ASIO_LIBNS::error_code(
...
int dev_poll_reactor::do_dev_poll_create()
{
  ...
  if (fd == -1)
  {
    ASIO_LIBNS::error_code ec(errno,
        ASIO_LIBNS::error::get_system_category());
    ASIO_LIBNS::detail::throw_error(ec, "/dev/poll");
  }
  return fd;
}
