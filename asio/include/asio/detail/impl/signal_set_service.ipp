...
# if defined(ASIO_HAS_IO_URING_AS_DEFAULT)
  pipe_read_op()
    : io_uring_operation(ASIO_LIBNS::error_code(), &pipe_read_op::do_prepare,
...
signal_set_service::signal_set_service(execution_context& context)
  : execution_context_service_base<signal_set_service>(context),
    scheduler_(ASIO_LIBNS::use_service<scheduler_impl>(context)),
#if !defined(ASIO_WINDOWS) \
  && !defined(ASIO_WINDOWS_RUNTIME) \
  && !defined(__CYGWIN__)
# if defined(ASIO_HAS_IO_URING_AS_DEFAULT)
...
# else // defined(ASIO_HAS_IO_URING_AS_DEFAULT)
    reactor_(ASIO_LIBNS::use_service<reactor>(context)),
# endif // defined(ASIO_HAS_IO_URING_AS_DEFAULT)
#endif // !defined(ASIO_WINDOWS)
       //   && !defined(ASIO_WINDOWS_RUNTIME)
       //   && !defined(__CYGWIN__)
...
# endif // defined(ASIO_HAS_IO_URING_AS_DEFAULT)
  ...
  case execution_context::fork_child:
    if (state->fork_prepared_)
    {
      ASIO_LIBNS::detail::signal_blocker blocker;
      close_descriptors();
      open_descriptors();
      int read_descriptor = state->read_descriptor_;
      state->fork_prepared_ = false;
...
ASIO_LIBNS::error_code signal_set_service::add(
    signal_set_service::implementation_type& impl,
    int signal_number, ASIO_LIBNS::error_code& ec)
{
...
ASIO_LIBNS::error_code signal_set_service::remove(
    signal_set_service::implementation_type& impl,
    int signal_number, ASIO_LIBNS::error_code& ec)
{
  // Check that the signal number is valid.
...
ASIO_LIBNS::error_code signal_set_service::clear(
    signal_set_service::implementation_type& impl,
...
ASIO_LIBNS::error_code signal_set_service::cancel(
    signal_set_service::implementation_type& impl,
    ASIO_LIBNS::error_code& ec)
{
...
void signal_set_service::open_descriptors()
{
  ...
  else
  {
    ASIO_LIBNS::error_code ec(errno,
        ASIO_LIBNS::error::get_system_category());
    ASIO_LIBNS::detail::throw_error(ec, "signal_set_service pipe");
  }
#endif // !defined(ASIO_WINDOWS)
       //   && !defined(ASIO_WINDOWS_RUNTIME)
       //   && !defined(__CYGWIN__)
