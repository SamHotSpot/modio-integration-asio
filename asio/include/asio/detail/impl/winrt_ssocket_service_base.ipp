    ...
    winrt_ssocket_service_base::base_implementation_type& other_impl)
{
  ...
  {
    // Insert implementation into linked list of all implementations.
    ASIO_LIBNS::detail::mutex::scoped_lock lock(other_service.mutex_);
...
ASIO_LIBNS::error_code winrt_ssocket_service_base::close(
    winrt_ssocket_service_base::base_implementation_type& impl,
...
std::size_t winrt_ssocket_service_base::do_get_endpoint(
    ...
    void* addr, std::size_t addr_len, ASIO_LIBNS::error_code& ec) const
{
  if (!is_open(impl))
  {
...
ASIO_LIBNS::error_code winrt_ssocket_service_base::do_set_option(
    winrt_ssocket_service_base::base_implementation_type& impl,
...
void winrt_ssocket_service_base::do_get_option(
    ...
    std::size_t* optlen, ASIO_LIBNS::error_code& ec) const
{
  if (!is_open(impl))
  {
    ec = ASIO_LIBNS::error::bad_descriptor;
...
ASIO_LIBNS::error_code winrt_ssocket_service_base::do_connect(
    winrt_ssocket_service_base::base_implementation_type& impl,
    const void* addr, ASIO_LIBNS::error_code& ec)
{
  if (!is_open(impl))
  {
    ec = ASIO_LIBNS::error::bad_descriptor;
    ...
    const void* addr, winrt_async_op<void>* op, bool is_continuation)
{
  if (!is_open(impl))
  {
    op->ec_ = ASIO_LIBNS::error::bad_descriptor;
  ...
  case ASIO_OS_DEF(AF_INET6):
    ...
    break;
  default:
    op->ec_ = ASIO_LIBNS::error::address_family_not_supported;
    break;
  }
...
std::size_t winrt_ssocket_service_base::do_send(
    ...
    socket_base::message_flags flags, ASIO_LIBNS::error_code& ec)
{
  if (flags)
  {
  ...
  try
  {
    buffer_sequence_adapter<ASIO_LIBNS::const_buffer,
      ASIO_LIBNS::const_buffer> bufs(ASIO_LIBNS::buffer(data));
...
void winrt_ssocket_service_base::start_send_op(
      winrt_ssocket_service_base::base_implementation_type& impl,
      const ASIO_LIBNS::const_buffer& data, socket_base::message_flags flags,
      winrt_async_op<unsigned int>* op, bool is_continuation)
{
  if (flags)
  {
    op->ec_ = ASIO_LIBNS::error::operation_not_supported;
    scheduler_.post_immediate_completion(op, is_continuation);
    return;
  }
  ...
  {
    op->ec_ = ASIO_LIBNS::error_code(e->HResult,
        ASIO_LIBNS::system_category());
    scheduler_.post_immediate_completion(op, is_continuation);
  }
}
    ...
    socket_base::message_flags flags, ASIO_LIBNS::error_code& ec)
{
  ...
  try
  {
    buffer_sequence_adapter<ASIO_LIBNS::mutable_buffer,
        ASIO_LIBNS::mutable_buffer> bufs(ASIO_LIBNS::buffer(data));
...
void winrt_ssocket_service_base::start_receive_op(
      winrt_ssocket_service_base::base_implementation_type& impl,
      const ASIO_LIBNS::mutable_buffer& data, socket_base::message_flags flags,
