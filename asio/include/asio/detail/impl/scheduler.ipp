...
scheduler::scheduler(ASIO_LIBNS::execution_context& ctx,
    int concurrency_hint, bool own_thread, get_task_func_type get_task)
  : ASIO_LIBNS::detail::execution_context_service_base<scheduler>(ctx),
    ...
    thread_(0)
{
  ...
  {
    ++outstanding_work_;
    ASIO_LIBNS::detail::signal_blocker sb;
    thread_ = new ASIO_LIBNS::detail::thread(thread_function(this));
  }
}
...
std::size_t scheduler::run(ASIO_LIBNS::error_code& ec)
{
...
std::size_t scheduler::run_one(ASIO_LIBNS::error_code& ec)
{
  ec = ASIO_LIBNS::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::wait_one(long usec, ASIO_LIBNS::error_code& ec)
{
  ec = ASIO_LIBNS::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::poll(ASIO_LIBNS::error_code& ec)
{
  ec = ASIO_LIBNS::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::poll_one(ASIO_LIBNS::error_code& ec)
{
  ec = ASIO_LIBNS::error_code();
  if (outstanding_work_ == 0)
  {
...
scheduler_task* scheduler::get_default_task(ASIO_LIBNS::execution_context& ctx)
{
#if defined(ASIO_HAS_IO_URING_AS_DEFAULT)
