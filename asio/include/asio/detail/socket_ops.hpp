...
ASIO_DECL int shutdown(socket_type s,
    int what, ASIO_LIBNS::error_code& ec);
...
ASIO_DECL bool sockatmark(socket_type s, ASIO_LIBNS::error_code& ec);

ASIO_DECL size_t available(socket_type s, ASIO_LIBNS::error_code& ec);

ASIO_DECL int listen(socket_type s,
    int backlog, ASIO_LIBNS::error_code& ec);
...
ASIO_DECL signed_size_type recv(socket_type s, buf* bufs,
    size_t count, int flags, ASIO_LIBNS::error_code& ec);

ASIO_DECL signed_size_type recv1(socket_type s,
    void* data, size_t size, int flags, ASIO_LIBNS::error_code& ec);
...
ASIO_DECL signed_size_type send1(socket_type s,
    const void* data, size_t size, int flags, ASIO_LIBNS::error_code& ec);

ASIO_DECL size_t sync_send(socket_type s, state_type state,
    const buf* bufs, size_t count, int flags,
    bool all_empty, ASIO_LIBNS::error_code& ec);
...
ASIO_DECL int poll_read(socket_type s,
    state_type state, int msec, ASIO_LIBNS::error_code& ec);
...
ASIO_DECL const char* inet_ntop(int af, const void* src, char* dest,
    size_t length, unsigned long scope_id, ASIO_LIBNS::error_code& ec);
...
ASIO_DECL int gethostname(char* name,
    int namelen, ASIO_LIBNS::error_code& ec);

#if !defined(ASIO_WINDOWS_RUNTIME)

ASIO_DECL ASIO_LIBNS::error_code getaddrinfo(const char* host,
    const char* service, const addrinfo_type& hints,
    addrinfo_type** result, ASIO_LIBNS::error_code& ec);

ASIO_DECL ASIO_LIBNS::error_code background_getaddrinfo(
...
ASIO_DECL ASIO_LIBNS::error_code getnameinfo(const void* addr,
    std::size_t addrlen, char* host, std::size_t hostlen, char* serv,
    std::size_t servlen, int flags, ASIO_LIBNS::error_code& ec);

ASIO_DECL ASIO_LIBNS::error_code sync_getnameinfo(const void* addr,
    std::size_t addrlen, char* host, std::size_t hostlen, char* serv,
    std::size_t servlen, int sock_type, ASIO_LIBNS::error_code& ec);

ASIO_DECL ASIO_LIBNS::error_code background_getnameinfo(
    const weak_cancel_token_type& cancel_token,
