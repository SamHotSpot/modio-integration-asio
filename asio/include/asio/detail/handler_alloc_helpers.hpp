...
{
  using ASIO_LIBNS::asio_handler_allocate;
  ...
  (void)static_cast<ASIO_LIBNS::asio_handler_allocate_is_no_longer_used>(
    asio_handler_allocate(static_cast<std::size_t>(0),
      ASIO_LIBNS::detail::addressof(h)));

  using ASIO_LIBNS::asio_handler_deallocate;
  // If you get an error here it is because some of your handlers still
  // overload asio_handler_deallocate, but this hook is no longer used.
  (void)static_cast<ASIO_LIBNS::asio_handler_deallocate_is_no_longer_used>(
    asio_handler_deallocate(static_cast<void*>(0),
      static_cast<std::size_t>(0), ASIO_LIBNS::detail::addressof(h)));
}
...
# if !defined(ASIO_DISABLE_SMALL_BLOCK_RECYCLING)
  return ASIO_LIBNS::detail::thread_info_base::allocate(
      ASIO_LIBNS::detail::thread_context::top_of_thread_call_stack(),
      s, align);
...
# endif // !defined(ASIO_DISABLE_SMALL_BLOCK_RECYCLING)
  ...
  (void)align;
  using ASIO_LIBNS::asio_handler_allocate;
  return asio_handler_allocate(s, ASIO_LIBNS::detail::addressof(h));
#endif
}
...
inline void deallocate(void* p, std::size_t s, Handler& h)
{
  ...
  (void)h;
#if !defined(ASIO_DISABLE_SMALL_BLOCK_RECYCLING)
  ASIO_LIBNS::detail::thread_info_base::deallocate(
      ASIO_LIBNS::detail::thread_context::top_of_thread_call_stack(), p, s);
#else // !defined(ASIO_DISABLE_SMALL_BLOCK_RECYCLING)
  (void)s;
  aligned_delete(p);
#endif // !defined(ASIO_DISABLE_SMALL_BLOCK_RECYCLING)
#else
  using ASIO_LIBNS::asio_handler_deallocate;
  asio_handler_deallocate(p, s, ASIO_LIBNS::detail::addressof(h));
#endif
}
...
#define ASIO_DEFINE_HANDLER_PTR(op) \
  ...
  { \
    ...
    { \
      typedef typename ::ASIO_LIBNS::associated_allocator< \
        Handler>::type associated_allocator_type; \
      typedef typename ::ASIO_LIBNS::detail::get_hook_allocator< \
        Handler, associated_allocator_type>::type hook_allocator_type; \
      ASIO_REBIND_ALLOC(hook_allocator_type, op) a( \
            ::ASIO_LIBNS::detail::get_hook_allocator< \
              Handler, associated_allocator_type>::get( \
                handler, ::ASIO_LIBNS::get_associated_allocator(handler))); \
      return a.allocate(1); \
    } \
    void reset() \
    { \
      ...
      { \
        ...
        ASIO_REBIND_ALLOC(hook_allocator_type, op) a( \
              ::ASIO_LIBNS::detail::get_hook_allocator< \
                Handler, associated_allocator_type>::get( \
                  *h, ::ASIO_LIBNS::get_associated_allocator(*h))); \
...
#define ASIO_DEFINE_TAGGED_HANDLER_ALLOCATOR_PTR(purpose, op) \
  ...
  { \
    ...
    { \
      typedef typename ::ASIO_LIBNS::detail::get_recycling_allocator< \
        Alloc, purpose>::type recycling_allocator_type; \
      ASIO_REBIND_ALLOC(recycling_allocator_type, op) a1( \
            ::ASIO_LIBNS::detail::get_recycling_allocator< \
              Alloc, purpose>::get(a)); \
...
#define ASIO_DEFINE_HANDLER_ALLOCATOR_PTR(op) \
  ASIO_DEFINE_TAGGED_HANDLER_ALLOCATOR_PTR( \
      ::ASIO_LIBNS::detail::thread_info_base::default_tag, op ) \
  /**/
