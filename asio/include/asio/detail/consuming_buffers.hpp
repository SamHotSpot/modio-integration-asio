...
public:
  ...
  prepared_buffers_type prepare(std::size_t max_size)
  {
    ...
    Buffer_Iterator next = ASIO_LIBNS::buffer_sequence_begin(buffers_);
    Buffer_Iterator end = ASIO_LIBNS::buffer_sequence_end(buffers_);
    ...
    while (next != end && max_size > 0 && (result.count) < result.max_buffers)
    {
      ...
      result.elems[result.count] = ASIO_LIBNS::buffer(next_buf, max_size);
...
public:
  ...
  {
    return ASIO_LIBNS::buffer(buffer_ + total_consumed_, max_size);
  }
...
public:
  ...
  boost::array<Buffer, 2> prepare(std::size_t max_size)
  {
    ...
    result[0] = ASIO_LIBNS::buffer(result[0] + total_consumed_, max_size);
    result[1] = ASIO_LIBNS::buffer(
...
class consuming_buffers<Buffer, null_buffers, const mutable_buffer*>
  : public ASIO_LIBNS::null_buffers
{
public:
