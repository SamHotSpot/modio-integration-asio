...
public:
  ...
  void dispatch(Function& function, Handler& handler)
  {
    executor_.dispatch(ASIO_MOVE_CAST(Function)(function),
        ASIO_LIBNS::get_associated_allocator(handler));
  }
...
public:
  ...
  explicit handler_work_base(int, int,
    ...
    : executor_(
        ex.target_type() == typeid(typename IoContext::executor_type)
          ? executor_type()
          : ASIO_LIBNS::prefer(ex, execution::outstanding_work.tracked))
#else // !defined(ASIO_NO_TYPEID)
    : executor_(ASIO_LIBNS::prefer(ex, execution::outstanding_work.tracked))
#endif // !defined(ASIO_NO_TYPEID)
  {
  ...
  {
    execution::execute(
        ASIO_LIBNS::prefer(executor_, execution::blocking.possibly),
        ASIO_MOVE_CAST(Function)(function));
  }
  ...
  handler_work(Handler& handler, const IoExecutor& io_ex) ASIO_NOEXCEPT
    : base1_type(0, 0, io_ex),
      base2_type(base1_type::owns_work(),
          ASIO_LIBNS::get_associated_executor(handler, io_ex), io_ex)
  {
  }
