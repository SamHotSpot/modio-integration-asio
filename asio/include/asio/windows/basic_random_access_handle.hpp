...
public:
   ...
   * @code
   * handle.write_some_at(42, ASIO_LIBNS::buffer(data, size));
   * @endcode
      ...
      const ConstBufferSequence& buffers)
  {
    ...
    ASIO_LIBNS::detail::throw_error(ec, "write_some_at");
    return s;
  }
   ...
   * To write a single data buffer use the @ref buffer function as follows:
   * @code
   * handle.async_write_some_at(42, ASIO_LIBNS::buffer(data, size), handler);
   * @endcode
   * See the @ref buffer documentation for information on writing multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
   ...
   * @code
   * handle.read_some_at(42, ASIO_LIBNS::buffer(data, size));
   * @endcode
      ...
      const MutableBufferSequence& buffers)
  {
    ...
    std::size_t s = this->impl_.get_service().read_some_at(
        this->impl_.get_implementation(), offset, buffers, ec);
    ASIO_LIBNS::detail::throw_error(ec, "read_some_at");
    return s;
  }
   ...
   * To read into a single data buffer use the @ref buffer function as follows:
   * @code
   * handle.async_read_some_at(42, ASIO_LIBNS::buffer(data, size), handler);
   * @endcode
   * See the @ref buffer documentation for information on reading into multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
