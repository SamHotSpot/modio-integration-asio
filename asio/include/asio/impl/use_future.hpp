...
public:
  ...
  {
    execution::execute(
        ASIO_LIBNS::require(system_executor(), Blocking()),
...
template <>
class promise_handler_selector<void(ASIO_LIBNS::error_code)>
...
template <typename Arg>
class promise_handler_selector<void(ASIO_LIBNS::error_code, Arg)>
...
#define ASIO_PRIVATE_PROMISE_SELECTOR_DEF(n) \
  ...
  template <typename Arg, ASIO_VARIADIC_TPARAMS(n)> \
  class promise_handler_selector< \
    void(ASIO_LIBNS::error_code, Arg, ASIO_VARIADIC_TARGS(n))> \
    ...
    promise_handler<Signature, Allocator>* h)
{
  typename promise_handler<Signature, Allocator>::executor_type
    ex(h->get_executor());
  ASIO_LIBNS::dispatch(ex, promise_function_wrapper<Function>(f));
}
...
struct equality_comparable<
    ASIO_LIBNS::detail::promise_executor<T, Blocking> >
{
...
struct execute_member<
    ASIO_LIBNS::detail::promise_executor<T, Blocking>, Function>
{
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ...
  >
{
  ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ASIO_STATIC_CONSTEXPR(bool, is_noexcept = true);
  typedef ASIO_LIBNS::detail::promise_executor<T,
