  ...
  void suspend_with(void (*fn)(void*), void* arg)
  {
    if (throw_if_cancelled_)
      if (!!cancellation_state_.cancelled())
        throw_error(ASIO_LIBNS::error::operation_aborted, "yield");
...
template <typename Executor, typename R>
class spawn_handler<Executor, R(ASIO_LIBNS::error_code)>
  : public spawn_handler_base<Executor>
{
public:
  typedef void return_type;
...
template <typename Executor, typename R, typename T>
class spawn_handler<Executor, R(ASIO_LIBNS::error_code, T)>
  : public spawn_handler_base<Executor>
{
public:
  typedef T return_type;
...
template <typename Executor, typename R, typename... Ts>
class spawn_handler<Executor, R(ASIO_LIBNS::error_code, Ts...)>
  : public spawn_handler_base<Executor>
{
public:
  typedef std::tuple<Ts...> return_type;
...
public:
  spawn_cancellation_handler(const Handler& handler, const Executor& ex)
    : ex_(ASIO_LIBNS::get_associated_executor(handler, ex))
  {
  }
    ...
    const boost::coroutines::attributes& attributes)
{
  ...
  ASIO_LIBNS::spawn(ex, ASIO_MOVE_CAST(Function)(function), attributes);
}
...
inline void spawn(const Executor& ex,
    ...
    >::type)
{
  ASIO_LIBNS::spawn(ASIO_LIBNS::strand<Executor>(ex),
...
inline void spawn(const strand<Executor>& ex,
    ...
    const boost::coroutines::attributes& attributes)
{
  ASIO_LIBNS::spawn(ASIO_LIBNS::bind_executor(
...
template <typename Function>
inline void spawn(const ASIO_LIBNS::io_context::strand& s,
    ASIO_MOVE_ARG(Function) function,
    const boost::coroutines::attributes& attributes)
{
  ASIO_LIBNS::spawn(ASIO_LIBNS::bind_executor(
...
{
  ASIO_LIBNS::spawn(ctx.get_executor(),
      ASIO_MOVE_CAST(Function)(function), attributes);
}
