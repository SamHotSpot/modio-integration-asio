...
public:
  ...
  redirect_error_handler(ASIO_LIBNS::error_code& ec,
  ...
  typename enable_if<
    !is_same<typename decay<Arg>::type, ASIO_LIBNS::error_code>::value
  >::type
...
struct redirect_error_signature<R(ASIO_LIBNS::error_code, Args...)>
{
  typedef R type(Args...);
};

template <typename R, typename... Args>
struct redirect_error_signature<R(const ASIO_LIBNS::error_code&, Args...)>
{
  typedef R type(Args...);
};
...
template <typename R, typename... Args>
struct redirect_error_signature<
  R(ASIO_LIBNS::error_code, Args...) noexcept>
{
...
template <typename R, typename... Args>
struct redirect_error_signature<
  R(const ASIO_LIBNS::error_code&, Args...) noexcept>
{
  typedef R type(Args...) & noexcept;
};
...
template <typename R>
struct redirect_error_signature<R(ASIO_LIBNS::error_code)>
{
  typedef R type();
};

template <typename R>
struct redirect_error_signature<R(const ASIO_LIBNS::error_code&)>
{
  typedef R type();
};
...
#define ASIO_PRIVATE_REDIRECT_ERROR_DEF(n) \
  ...
  struct redirect_error_signature< \
      R(ASIO_LIBNS::error_code, ASIO_VARIADIC_TARGS(n))> \
  { \
  ...
  template <typename R, ASIO_VARIADIC_TPARAMS(n)> \
  struct redirect_error_signature< \
      R(const ASIO_LIBNS::error_code&, ASIO_VARIADIC_TARGS(n))> \
  { \
    typedef R type(ASIO_VARIADIC_TARGS(n)); \
  }; \
  /**/
...
template <typename R>
struct redirect_error_signature<
  R(ASIO_LIBNS::error_code) noexcept>
{
...
template <typename R>
struct redirect_error_signature<
  R(const ASIO_LIBNS::error_code&) noexcept>
{
  typedef R type() noexcept;
};
...
#define ASIO_PRIVATE_REDIRECT_ERROR_DEF(n) \
  template <typename R, ASIO_VARIADIC_TPARAMS(n)> \
  struct redirect_error_signature< \
      R(ASIO_LIBNS::error_code, ASIO_VARIADIC_TARGS(n)) noexcept> \
  { \
...
struct async_result<redirect_error_t<CompletionToken>, Signature>
{
  ...
  struct init_wrapper
  {
    ...
    init_wrapper(ASIO_LIBNS::error_code& ec, ASIO_MOVE_ARG(Init) init)
      : ec_(ec),
