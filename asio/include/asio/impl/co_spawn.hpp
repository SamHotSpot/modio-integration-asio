    ...
    awaitable<T, Executor>*, Executor ex, F f, Handler handler)
{
  ...
  auto handler_work = make_co_spawn_work_guard(
      ASIO_LIBNS::get_associated_executor(handler, ex));
...
public:
  ...
  void operator()(cancellation_type_t type)
  {
    ...
    ASIO_LIBNS::dispatch(ex_, [sig, type]{ sig->emit(type); });
  }
...
public:
  ...
  void operator()(Handler&& handler, F&& f) const
  {
    ...
    auto slot = ASIO_LIBNS::get_associated_cancellation_slot(handler);
