...
namespace detail
{
  struct default_connect_condition
  {
    ...
    bool operator()(const ASIO_LIBNS::error_code&, const Endpoint&)
    {
      return true;
    }
  };
  ...
  inline typename Protocol::endpoint deref_connect_result(
      Iterator iter, ASIO_LIBNS::error_code& ec)
  {
    ...
    static const bool value =
      sizeof(asio_connect_condition_check(
        (declval<legacy_connect_condition_helper<T, Iterator> >())(
          declval<const ASIO_LIBNS::error_code>(),
  ...
  inline Iterator call_connect_condition(ConnectCondition& connect_condition,
      const ASIO_LIBNS::error_code& ec, Iterator next, Iterator end,
...
namespace detail
{
  ...
  protected:
    ...
    void check_condition(const ASIO_LIBNS::error_code& ec,
  ...
  protected:
    ...
    template <typename Iterator>
    void check_condition(const ASIO_LIBNS::error_code&, Iterator&, Iterator&)
    {
    }
  };
  ...
  public:
    ...
    void operator()(ASIO_LIBNS::error_code ec, int start = 0)
    {
  ...
  //private:
    template <typename Iterator>
    void process(ASIO_LIBNS::error_code ec,
      ...
      switch (start_ = start)
      {
        ...
        for (;;)
        {
          ...
          if (start)
          {
            ec = ASIO_LIBNS::error::not_found;
            ASIO_HANDLER_LOCATION((__FILE__, __LINE__, "async_connect"));
            ASIO_LIBNS::post(socket_.get_executor(),
                detail::bind_handler(
        ...
        const ConnectCondition& connect_condition) const
    {
      ...
      range_connect_op<Protocol, Executor, EndpointSequence, ConnectCondition,
        typename decay<RangeConnectHandler>::type>(socket_, endpoints,
          connect_condition, handler2.value)(ASIO_LIBNS::error_code(), 1);
    }
