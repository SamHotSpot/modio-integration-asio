...
public:
  ...
  void* operator new(std::size_t size)
  {
    return ASIO_LIBNS::detail::thread_info_base::allocate(
        ASIO_LIBNS::detail::thread_info_base::awaitable_frame_tag(),
  ...
  void set_error(const ASIO_LIBNS::error_code& ec)
  {
    this->set_except(std::make_exception_ptr(ASIO_LIBNS::system_error(ec)));
  }
  ...
  auto await_transform(awaitable<T, Executor> a) const
  {
    if (attached_thread_->entry_point()->throw_if_cancelled_)
      if (!!attached_thread_->get_cancellation_state().cancelled())
        throw_error(ASIO_LIBNS::error::operation_aborted, "co_await");
    return a;
  }
...
private:
  ...
  ASIO_LIBNS::cancellation_slot parent_cancellation_slot_;
  ASIO_LIBNS::cancellation_state cancellation_state_;
  bool has_executor_;
...
template <typename R, typename Executor>
class awaitable_async_op_handler<R(ASIO_LIBNS::error_code), Executor>
  : public awaitable_thread<Executor>
{
public:
  typedef ASIO_LIBNS::error_code* result_type;
...
template <typename R, typename T, typename Executor>
class awaitable_async_op_handler<R(ASIO_LIBNS::error_code, T), Executor>
  : public awaitable_thread<Executor>
{
public:
  struct result_type
  ...
  void operator()(ASIO_LIBNS::error_code ec, T value)
  {
    result_.ec_ = &ec;
...
template <typename R, typename... Ts, typename Executor>
class awaitable_async_op_handler<R(ASIO_LIBNS::error_code, Ts...), Executor>
  : public awaitable_thread<Executor>
{
public:
  struct result_type
  ...
  template <typename... Args>
  void operator()(ASIO_LIBNS::error_code ec, Args&&... args)
  {
    result_.ec_ = &ec;
...
struct coroutine_traits<ASIO_LIBNS::awaitable<T, Executor>, Args...>
{
  typedef ASIO_LIBNS::detail::awaitable_frame<T, Executor> promise_type;
};
