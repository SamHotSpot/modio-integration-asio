...
/**
 * @defgroup connect ASIO_LIBNS::connect
 *
 ...
 *
 * @throws ASIO_LIBNS::system_error Thrown on failure. If the sequence is
 * empty, the associated @c error_code is ASIO_LIBNS::error::not_found.
 ...
 * ASIO_LIBNS::connect(s, r.resolve(q)); @endcode
 */
 ...
 * empty, set to ASIO_LIBNS::error::not_found. Otherwise, contains the error
 ...
 * ASIO_LIBNS::connect(s, r.resolve(q), ec);
 * if (ec)
 * {
 *   // An error occurred.
 * } @endcode
...
template <typename Protocol, typename Executor, typename EndpointSequence>
typename Protocol::endpoint connect(basic_socket<Protocol, Executor>& s,
    const EndpointSequence& endpoints, ASIO_LIBNS::error_code& ec,
    typename constraint<is_endpoint_sequence<
        EndpointSequence>::value>::type = 0);
 ...
 * iterator types such as @c ASIO_LIBNS::ip::tcp::resolver::iterator.
 */
...
Iterator connect(basic_socket<Protocol, Executor>& s,
    Iterator begin, ASIO_LIBNS::error_code& ec,
    typename constraint<!is_endpoint_sequence<Iterator>::value>::type = 0);
 ...
 * tcp::socket s(my_context);
 * ASIO_LIBNS::connect(s, e.begin(), e.end()); @endcode
 */
template <typename Protocol, typename Executor, typename Iterator>
Iterator connect(basic_socket<Protocol, Executor>& s,
 ...
 * tcp::socket s(my_context);
 * ASIO_LIBNS::error_code ec;
 * ASIO_LIBNS::connect(s, e.begin(), e.end(), ec);
 * if (ec)
 * {
 *   // An error occurred.
 * } @endcode
...
template <typename Protocol, typename Executor, typename Iterator>
Iterator connect(basic_socket<Protocol, Executor>& s,
    Iterator begin, Iterator end, ASIO_LIBNS::error_code& ec);
 ...
 *   }
 * }; @endcode
 * It would be used with the ASIO_LIBNS::connect function as follows:
 * @code tcp::resolver r(my_context);
 * tcp::resolver::query q("host", "service");
 * tcp::socket s(my_context);
 * tcp::endpoint e = ASIO_LIBNS::connect(s,
...
template <typename Protocol, typename Executor,
    typename Iterator, typename ConnectCondition>
Iterator connect(basic_socket<Protocol, Executor>& s, Iterator begin,
    ConnectCondition connect_condition, ASIO_LIBNS::error_code& ec,
    typename constraint<!is_endpoint_sequence<Iterator>::value>::type = 0);
#endif // !defined(ASIO_NO_DEPRECATED)
 ...
 * tcp::resolver::results_type e = r.resolve(q);
 * tcp::socket s(my_context);
 * tcp::resolver::results_type::iterator i = ASIO_LIBNS::connect(
...
/**
 * @defgroup async_connect ASIO_LIBNS::async_connect
 *
 ...
 *   // ASIO_LIBNS::error::not_found. Otherwise, contains the
 ...
 * @code void(ASIO_LIBNS::error_code, typename Protocol::endpoint) @endcode
 *
 * @par Example
 * @code tcp::resolver r(my_context);
 * tcp::resolver::query q("host", "service");
 ...
 *   if (!ec)
 *   {
 *     ASIO_LIBNS::async_connect(s, results, connect_handler);
 *   }
 * }
 *
 * // ...
...
ASIO_INITFN_AUTO_RESULT_TYPE_PREFIX(RangeConnectToken,
    void (ASIO_LIBNS::error_code, typename Protocol::endpoint))
 ...
 *
 * @par Completion Signature
 * @code void(ASIO_LIBNS::error_code, Iterator) @endcode
 *
 * @note This overload assumes that a default constructed object of type @c
 * Iterator represents the end of the sequence. This is a valid assumption for
 * iterator types such as @c ASIO_LIBNS::ip::tcp::resolver::iterator.
...
ASIO_INITFN_AUTO_RESULT_TYPE_PREFIX(IteratorConnectToken,
    void (ASIO_LIBNS::error_code, Iterator))
 ...
 *   if (!ec)
 *   {
 *     ASIO_LIBNS::async_connect(s, results,
 ...
 *     ASIO_LIBNS::async_connect(s, i, end,
 *         my_connect_condition(),
 *         connect_handler);
 *   }
 * }
