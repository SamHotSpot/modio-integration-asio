 ...
 * ASIO_LIBNS::signal_set signals(my_context, SIGINT, SIGTERM);
 *
...
public:
   ...
   * @code ASIO_LIBNS::signal_set signals(ex);
    ...
    : impl_(0, ex)
  {
    ...
    ASIO_LIBNS::detail::throw_error(ec, "add");
  }
   ...
   *
   * @note This constructor is equivalent to performing:
   * @code ASIO_LIBNS::signal_set signals(context);
   * signals.add(signal_number_1); @endcode
   */
  template <typename ExecutionContext>
  ...
  void remove(int signal_number)
  {
    ...
    ASIO_LIBNS::detail::throw_error(ec, "remove");
  }
  ...
  void clear()
  {
    ...
    ASIO_LIBNS::detail::throw_error(ec, "clear");
  }
   ...
   * @note Removes all queued notifications.
   */
  ASIO_SYNC_OP_VOID clear(ASIO_LIBNS::error_code& ec)
  {
    impl_.get_service().clear(impl_.get_implementation(), ec);
    ASIO_SYNC_OP_VOID_RETURN(ec);
  }
   ...
   * operation will be invoked with the ASIO_LIBNS::error::operation_aborted
   * error code.
   *
   ...
   * error code ASIO_LIBNS::error::operation_aborted.
   *
   ...
   * @code void(ASIO_LIBNS::error_code, int) @endcode
   *
   ...
   */
  template <
    ASIO_COMPLETION_TOKEN_FOR(void (ASIO_LIBNS::error_code, int))
  ...
  async_wait(
    ...
    ASIO_INITFN_AUTO_RESULT_TYPE_SUFFIX((
      async_initiate<SignalToken, void (ASIO_LIBNS::error_code, int)>(
          declval<initiate_async_wait>(), token)))
  {
    return async_initiate<SignalToken, void (ASIO_LIBNS::error_code, int)>(
