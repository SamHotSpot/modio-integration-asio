...
private:
  ...
  auto invoke_helper(
    ...
    -> decltype(
        ASIO_LIBNS::async_initiate<CompletionToken, void(Values...)>(
          initiate(), token,
          std::get<I>(
            ASIO_MOVE_CAST(std::tuple<Values...>)(this->values_))...))
  {
    return ASIO_LIBNS::async_initiate<CompletionToken, void(Values...)>(
        initiate(), token,
 ...
 * operation, typically using the special value @c ASIO_LIBNS::deferred. For
 * example:
 *
 * @code auto my_deferred_op
 *   = my_socket.async_read_some(my_buffer,
 *       ASIO_LIBNS::deferred); @endcode
 *
...
/**
 * See the documentation for ASIO_LIBNS::deferred_t for a usage example.
 */
