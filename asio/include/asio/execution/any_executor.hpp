...
public:
   ...
   * auto ex2 = ASIO_LIBNS::requre(ex, execution::blocking.possibly); @endcode
   */
   ...
   * size_t n = ASIO_LIBNS::query(ex, execution::occupancy); @endcode
   */
  template <typename Property>
   ...
   *
   * Throws ASIO_LIBNS::bad_executor if the polymorphic wrapper has no target.
   */
...
public:
  ...
  any_executor_base(Executor other, true_type)
    : object_fns_(object_fns_table<ASIO_LIBNS::detail::shared_ptr<void> >()),
      target_fns_(other.target_fns_)
  {
    ASIO_LIBNS::detail::shared_ptr<Executor> p =
      ASIO_LIBNS::detail::make_shared<Executor>(
          ASIO_MOVE_CAST(Executor)(other));
    target_ = p->template target<void>();
    new (&object_) ASIO_LIBNS::detail::shared_ptr<void>(
        ASIO_MOVE_CAST(ASIO_LIBNS::detail::shared_ptr<Executor>)(p));
  }
  ...
  void execute(ASIO_MOVE_ARG(F) f) const
  {
    ...
    {
      ASIO_LIBNS::detail::non_const_lvalue<F> f2(f);
...
protected:
  ...
  {
    typedef ASIO_LIBNS::detail::shared_ptr<void> type;
    ex.object<type>().~type();
  }
  ...
  static const object_fns* object_fns_table(
      typename enable_if<
        is_same<Obj, ASIO_LIBNS::detail::shared_ptr<void> >::value
      >::type* = 0)
  {
    static const object_fns fns =
    {
  ...
  typedef ASIO_LIBNS::detail::executor_function function;
  typedef ASIO_LIBNS::detail::executor_function_view function_view;
...
# pragma warning (disable:4702)
  ...
  static void query_fn_non_void(void*, const void* ex, const void* prop,
      typename enable_if<
        ASIO_LIBNS::can_query<const Ex&, const Prop&>::value
          && is_same<typename Prop::polymorphic_query_result_type, void>::value
      >::type*)
  {
    ASIO_LIBNS::query(*static_cast<const Ex*>(ex),
  ...
  static Poly require_fn_impl(const void* ex, const void* prop,
      ...
      >::type*)
  {
    return ASIO_LIBNS::require(*static_cast<const Ex*>(ex),
        *static_cast<const Prop*>(prop));
  }
  ...
  static Poly prefer_fn_impl(const void* ex, const void* prop,
      ...
      >::type*)
  {
    return ASIO_LIBNS::prefer(*static_cast<const Ex*>(ex),
        *static_cast<const Prop*>(prop));
  }
...
private:
  ...
  {
    return ASIO_LIBNS::query(ex, execution::blocking);
  }
...
//  template <typename...> friend class any_executor;
  ...
  typedef aligned_storage<
      sizeof(ASIO_LIBNS::detail::shared_ptr<void>) + sizeof(void*),
      alignment_of<ASIO_LIBNS::detail::shared_ptr<void> >::value
    >::type object_type;
...
public:
      ...
      >::type* = 0) const
  {
    ...
    prop_fns_[found::index].query(&result, object_fns_->target(*this),
        &static_cast<const typename found::type&>(p));
    return *ASIO_LIBNS::detail::scoped_ptr<
      typename found::query_result_type>(result);
  }
