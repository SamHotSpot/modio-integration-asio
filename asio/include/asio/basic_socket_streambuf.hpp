...
# endif // !defined(ASIO_NO_DEPRECATED)
      ...
      expiry_time_(other.expiry_time_)
  {
    ...
    other.ec_ = ASIO_LIBNS::error_code();
   ...
   * buffer.
   */
  const ASIO_LIBNS::error_code& error() const
  {
    return ec_;
  }
   ...
   * buffer.
   */
  const ASIO_LIBNS::error_code& puberror() const
  {
    return error();
  }
   ...
   * ASIO_LIBNS::error::operation_aborted.
   *
...
protected:
  int_type underflow()
  {
    ...
    ec_ = ASIO_LIBNS::error::operation_not_supported;
    ...
    for (;;)
    {
      ...
      {
        ec_ = ASIO_LIBNS::error::timed_out;
        return traits_type::eof();
      }
      ...
      detail::buffer_sequence_adapter<mutable_buffer, mutable_buffer>
        bufs(ASIO_LIBNS::buffer(get_buffer_) + putback_max);
      ...
      if (bytes == 0)
      {
        ec_ = ASIO_LIBNS::error::eof;
        return traits_type::eof();
      }

      // Operation failed.
      if (ec_ != ASIO_LIBNS::error::would_block
          && ec_ != ASIO_LIBNS::error::try_again)
        return traits_type::eof();
  ...
  int_type overflow(int_type c)
  {
    ...
    if (put_buffer_.empty())
    {
      ...
      output_buffer = ASIO_LIBNS::buffer(&ch, sizeof(char_type));
    }
    else
    {
      output_buffer = ASIO_LIBNS::buffer(pbase(),
...
private:
  ...
  void connect_to_endpoints(EndpointIterator begin, EndpointIterator end)
  {
    ...
    ec_ = ASIO_LIBNS::error::not_found;
    for (EndpointIterator i = begin; i != end; ++i)
    {
      ...
      // Operation failed.
      if (ec_ != ASIO_LIBNS::error::in_progress
          && ec_ != ASIO_LIBNS::error::would_block)
        continue;
      ...
      ec_ = ASIO_LIBNS::error_code(connect_error,
