...
{
  if (new_size > sizeof(ASIO_LIBNS::detail::sockaddr_un_type))
  {
  ...
  else
  {
    path_length_ = new_size
      - offsetof(ASIO_LIBNS::detail::sockaddr_un_type, sun_path);
...
void endpoint::init(const char* path_name, std::size_t path_length)
{
  if (path_length > sizeof(data_.local.sun_path) - 1)
  {
    ...
    ASIO_LIBNS::error_code ec(ASIO_LIBNS::error::name_too_long);
    ASIO_LIBNS::detail::throw_error(ec);
  }

  using namespace std; // For memset and memcpy.
  memset(&data_.local, 0, sizeof(ASIO_LIBNS::detail::sockaddr_un_type));
