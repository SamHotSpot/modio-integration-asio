...
public:
  ...
  std::size_t fill(ASIO_LIBNS::error_code& ec);
  ...
  /// Determine the amount of data that may be read without blocking.
  std::size_t in_avail(ASIO_LIBNS::error_code& ec)
  {
...
private:
  ...
  {
    std::size_t bytes_copied = ASIO_LIBNS::buffer_copy(
  ...
  {
    return ASIO_LIBNS::buffer_copy(buffers, storage_.data(), storage_.size());
  }
